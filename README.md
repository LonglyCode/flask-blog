...
## 前言
    ...
    参考《Flask Web开发》一书进行个人blog开发，项目组织结构也是直接借用flasky的结构。
...
## 全文搜索
可以看到本博客尽量用python本身来实现除前端外的所有功能，不想引入谷歌搜索或者使用其他语言实现的全文索引引擎，`whoosh`是纯python 实现的全文索引，又因为要涉及到中文，所以肯定还要结合`jieba`分词，还好这两者结合非常好。
### whoosh_search
